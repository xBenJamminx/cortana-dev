...
## API Credentials — USE THESE, DO NOT GUESS

All three integrations use **Composio connectedAccountIds**. These are OAuth connections managed by Composio. Do NOT use raw API keys from .env for this workflow. Do NOT claim these are expired or invalid without actually trying them first — if a call fails, show the actual error message.
...
**IMPORTANT:** These are NOT raw API keys. They are Composio-managed OAuth connections. Use them via Composio tool calls (SLACK_FETCH_CONVERSATION_HISTORY, NOTION_QUERY_DATABASE, NOTION_INSERT_ROW_DATABASE, NOTION_UPDATE_PAGE, GOOGLESHEETS_BATCH_GET, GOOGLESHEETS_BATCH_UPDATE). If you get an error, report the ACTUAL error — do not assume credentials are broken.
...
### STEP 3.5 — APPROVAL GATE (mandatory)

**STOP HERE.** Present the full delta list to Ben before making any changes:

1. Show every proposed change — new tasks, status updates, and the evidence behind each one
2. Format it clearly so Ben can scan and approve/reject individual items
3. **Wait for Ben's explicit go-ahead** before proceeding to Step 4
4. If Ben corrects anything, update the delta list before continuing

Do NOT skip this step. Do NOT batch-update and report after. Ben reviews first, then you write.
