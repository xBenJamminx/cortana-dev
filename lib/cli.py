...
def main():
    ...
    # Tail command (daemon mode)
    tail_parser = subparsers.add_parser('tail', help='Poll for live updates (daemon mode)')
    tail_parser.add_argument('--chat-id', type=int, action='append', help='Chat ID(s) to monitor')
    tail_parser.add_argument('-i', '--interval', type=int, default=5, help='Poll interval in seconds (default: 5)')
