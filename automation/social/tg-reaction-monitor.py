...
TOPICS = [1, 20, 22, 26, 29, 31, 1720, 2122]
TYPING_INTERVAL = 4
EYES = '👀'
...
def main():
    ...
    for line in proc.stdout:
        ...
        if '[agent/claude-cli] cli exec' in line:
            ...
            # If processing has been stuck for >120s without a response, auto-reset
            if processing and (now - processing_start) > 300:
                log(f'Processing exceeded 5m - {now - processing_start:.0f}s, resetting')
                stop_processing()
