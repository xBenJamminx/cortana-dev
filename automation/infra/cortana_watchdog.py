...
from core.monitoring.alerting import send_alert, clear_alert
...
CLAUDE_SESSION_DIRS = [
    ...
    Path("/root/.claude/projects/-root-clawd"),
    Path("/root/.claude/projects/-root"),
    Path("/root/.claude/projects/-root--openclaw-workspace-worker"),
]


def session_exists_in_claude(session_id: str) -> bool:
    """Check if a Claude CLI session file exists in any project directory."""
...
def _nuke_stale_ids(obj, session_id, path_str="", cleaned=None):
    """Recursively find and clear all references to a stale session ID."""
    if cleaned is None:
        cleaned = []
    if isinstance(obj, dict):
        for k in list(obj.keys()):
            v = obj[k]
            if k in ("sessionId", "claudeCliSessionId", "sessionFile") and isinstance(v, str) and session_id in v:
                cleaned.append(f"{path_str}.{k}")
                obj[k] = ""
            elif k in ("cliSessionIds", "backendSessions") and isinstance(v, dict):
                stale_sks = [sk for sk in v if isinstance(v[sk], str) and session_id in v[sk]]
                for sk in stale_sks:
                    cleaned.append(f"{path_str}.{k}.{sk}")
                    v[sk] = ""
            elif k == "systemPromptReport" and isinstance(v, dict) and isinstance(v.get("sessionId"), str) and session_id in v.get("sessionId", ""):
                cleaned.append(f"{path_str}.{k}.sessionId")
                v["sessionId"] = ""
            elif isinstance(v, (dict, list)):
                _nuke_stale_ids(v, session_id, f"{path_str}.{k}", cleaned)
    elif isinstance(obj, list):
        for i, v in enumerate(obj):
            if isinstance(v, (dict, list)):
                _nuke_stale_ids(v, session_id, f"{path_str}[{i}]", cleaned)
    return cleaned


def check_stale_sessions():
    """Detect and clear stale Claude CLI session IDs from openclaw sessions.json.

    When openclaw caches a Claude CLI session ID that no longer exists,
    every attempt to --resume it fails with 'No conversation found'.
    This function finds and clears those stale CLI references so openclaw
    creates fresh sessions instead of looping on dead ones.

    IMPORTANT: Only checks claudeCliSessionId and cliSessionIds fields.
    Do NOT touch sessionId or systemPromptReport.sessionId - those are
    openclaw internal IDs and will never have .jsonl files on disk.
    """
    ...
    for key, entry in data.items():
        ...
        # Only check CLI session IDs - these are what --resume uses
        cli_sid = entry.get("claudeCliSessionId", "")
        if isinstance(cli_sid, str) and cli_sid and not session_exists_in_claude(cli_sid):
            all_cleaned.append("%s.claudeCliSessionId (was %s...)" % (key, cli_sid[:12]))
            entry["claudeCliSessionId"] = ""
            changed = True

        cli_ids = entry.get("cliSessionIds", {})
        if isinstance(cli_ids, dict):
            for backend, sid in list(cli_ids.items()):
                if isinstance(sid, str) and sid and not session_exists_in_claude(sid):
                    all_cleaned.append("%s.cliSessionIds.%s (was %s...)" % (key, backend, sid[:12]))
                    cli_ids[backend] = ""
                    changed = True

    if changed and all_cleaned:
        try:
            SESSIONS_JSON.write_text(json.dumps(data, indent=2))
            log.warning("Cleared %d stale CLI session ref(s): %s", len(all_cleaned), "; ".join(all_cleaned))
            details = "; ".join(all_cleaned[:5])
            send_alert(
                "stale_sessions_cleaned",
                "Auto-cleared %d stale CLI session ref(s): %s" % (len(all_cleaned), details),
                level="info",
                cooldown=600,
            )
        except OSError as e:
            log.error("Failed to write cleaned sessions.json: %s", e)
...
def main():
    ...
    # check_stale_sessions()  # DISABLED: was corrupting sessions.json, CLI update was the real fix
    check_log_staleness()
